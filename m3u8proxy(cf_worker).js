      ...
      if (line.startsWith("#")) {
        if (line.startsWith("#EXT-X-KEY:")) {
          ...
          const newUrl = `/ts-proxy?url=${encodeURIComponent(
            keyUrl
          )}&headers=${encodeURIComponent(JSON.stringify(fetchHeaders))}`;
        ...
        try {
          ...
          // If line is not a valid URL, keep it as is
          newLines.push(line);
        }
      } else {
        newLines.push(line);
...
async function handleTsProxy(request) {
  ...
  try {
    // Merge default headers with provided headers (User-Agent takes precedence)
    const fetchHeaders = {
      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.132 Safari/537.36",
      "Referer": "http://www.fawanews.sc/",
      "Origin": "http://www.fawanews.sc/",
